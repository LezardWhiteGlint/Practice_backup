...
def daily_display(daily_h,date):
    ...
    for i in range(len(date_new)):
        hours = int(time_new[i]/2)
        minutes = int(time_new[i]%2)*30
        message = str(hours)+':'+str(minutes)
        print('Date: '+date_new[i]+'-------'+message)
        print "\n"
...
def app():
    ...
    daily_h = daily_hours(time_cal)
    daily_display(daily_h,date)
    display(time_result)
